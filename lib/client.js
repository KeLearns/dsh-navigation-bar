  ...
  window.__ModuleLoader__.load({
    ...
    factory: (require) => {
      ...
      function PianoNavOverlayInner(props) {
        ...
        // 锚点行元素缓存（key → element）：避免每次重算对每个 key 做 querySelector。
        // 行可能被虚拟滚动回收重建，使用前校验 isConnected。
        const rowCacheRef = useRef(null)
        // scrollport 的 ResizeObserver（惰性附加，见 attachObserver）。
        ...
        const compute = () => {
          ...
          // 行引用走缓存：一次 querySelectorAll 收集全部锚点行（O(n)），
          // 避免对每个 key 单独 querySelector（O(n²)）并强制同步布局。
        ...
        // 惰性附加 scrollport 的 ResizeObserver：compute() 每次运行时都会
        // 调用（幂等），因此无论 scrollport 何时挂载（会话视图切换、迟到
        // 渲染），只要有任何重算触发点就能补挂观察，无需常驻定时器。
        // 切换会话时 scrollport 会被重建为新元素：观察目标与记录不同则
        // 先解除旧目标再观察新目标。
        ...
        //   scroll / resize / scrollport size changes → recompute (cheap, rAF-merged)
        //   entries signature change (user message set) → recompute
        // There is no document.body MutationObserver and no perpetual interval —
        // DSH's live UI mutates the DOM constantly (streaming tokens, cursor,
        // animations), and reacting to every mutation burned ~50% of a core in
        // the renderer process. The scrollport itself may mount late (root-scope
        // overlay mounts before the session view); attachObserver above heals
        // lazily on any subsequent recompute, so no background timer is needed.
        useEffect(() => {
        ...
        // 仅在用户消息集合（entries 签名）变化时重算布局——模型流式输出
        // 只改 modelText，不应触发钢琴键重排。
